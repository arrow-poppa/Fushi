...
void main() {
  ...
  group('stringifyMessageContent', () {
    ...
    test('concatenates array parts with no separator', () {
      ...
      expect(
        AiResponseParser.stringifyMessageContent(
          json('''
    ...
    test('accepts bare strings inside the array', () {
      expect(AiResponseParser.stringifyMessageContent(json('["a","b"]')), 'ab');
    });
    ...
    test('reads a bare object', () {
      expect(
        AiResponseParser.stringifyMessageContent(json('{"text":"x"}')),
        'x',
      );
      expect(
        AiResponseParser.stringifyMessageContent(json('{"content":"y"}')),
        'y',
      );
    });
    ...
    test('yields empty for shapes it does not recognise', () {
      ...
      expect(
        AiResponseParser.stringifyMessageContent(json('[{"foo":1}]')),
        isEmpty,
      );
      expect(
        AiResponseParser.stringifyMessageContent(json('[null,1,true]')),
        isEmpty,
      );
    });
  });
  ...
  group('extractOpenAiStreamDelta', () {
    ...
    test('ignores a reasoning field alongside real content', () {
      expect(
        AiResponseParser.extractOpenAiStreamDelta(
          json('{"choices":[{"delta":{"reasoning":"hmm","content":"real"}}]}'),
        ),
        'real',
      );
    });
    ...
    test('yields empty for malformed payloads instead of throwing', () {
      for (final String source in <String>[
        ...
        expect(
          AiResponseParser.extractOpenAiStreamDelta(json(source)),
          isEmpty,
  ...
  group('extractOpenAiMessage', () {
    ...
    test('prefers message.content over text', () {
      expect(
        AiResponseParser.extractOpenAiMessage(
          json('{"choices":[{"message":{"content":"msg"},"text":"legacy"}]}'),
        ),
        'msg',
      );
    });
      ...
      expect(
        ...
        throwsA(
          isA<AiRequestException>().having(
            (AiRequestException e) => e.message,
            'message',
            'quota exceeded',
          ),
        ),
    ...
    test('yields empty for an empty or malformed body', () {
      ...
      expect(
        AiResponseParser.extractOpenAiMessage(json('{"choices":[]}')),
        isEmpty,
      );
  ...
  group('extractGeminiText', () {
    test('concatenates every non-thought part', () {
      expect(
        AiResponseParser.extractGeminiText(
          json('''
    ...
    test(
      'reads past a leading thought part rather than stopping at index 0',
      () {
    ...
    test('yields empty for malformed payloads instead of throwing', () {
      for (final String source in <String>[
        ...
        expect(
          AiResponseParser.extractGeminiText(json(source)),
          isEmpty,
  ...
  group('extractErrorMessage', () {
    ...
    test('reads a bare string error and a top-level message', () {
      expect(
        AiResponseParser.extractErrorMessage(json('{"error":"oops"}')),
        'oops',
      );
      expect(
        AiResponseParser.extractErrorMessage(json('{"message":"nope"}')),
        'nope',
      );
    });
    ...
    test('returns null when there is nothing usable', () {
      ...
      expect(
        AiResponseParser.extractErrorMessage(json('{"error":{}}')),
        isNull,
      );
